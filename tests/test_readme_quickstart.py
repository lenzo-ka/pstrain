...
def test_checkout_quickstart_fixture_paths_exist() -> None:
    ...
    guide_path = "docs/getting-started.md"
    assert f"/blob/main/{guide_path})" in readme
    guide = (root / guide_path).read_text()
    checkout = guide.split("## Train a small model", 1)[1].split("\n## ", 1)[0]
    fixture_paths = re.findall(r"tests/fixtures/[\w./-]+", checkout)

    assert len(fixture_paths) >= 5
    for relative_path in fixture_paths:
        assert (root / relative_path).exists(), f"Quick-start path does not exist: {relative_path}"
