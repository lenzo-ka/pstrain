"""Task definitions for the pstrain training pipeline.
...
Targets exposed to `pstrain build`:
...
from pstrain.lib.pipeline.targets import DEFAULT_TARGET, TARGETS, TargetSpec

__all__ = ["DEFAULT_TARGET", "TARGETS", "TargetSpec", "build_pipeline"]
...
def _make_questions_task(ctx: PipelineContext) -> Task:
    ...
    def run() -> None:
        from pstrain.lib.steps.cd_pipeline import run_make_questions
...
def _make_prune_trees_task(ctx: PipelineContext) -> Task:
    ...
    def run() -> None:
        from pstrain.lib.steps.cd_pipeline import run_prune_trees
...
def _make_cd_1g_init_task(ctx: PipelineContext) -> Task:
    ...
    def run() -> None:
        from pstrain.lib.steps.cd_pipeline import run_init_cd_tied, run_tiestate
...
def _make_test_task(ctx: PipelineContext, *, model: str) -> Task:
    ...
    def run() -> None:
        from pstrain.lib.testing import create_report, load_transcripts, test_model
...
def build_pipeline(ctx: PipelineContext) -> Pipeline:
    ...
    # Register named targets so users can type `pstrain build cd-8g`.
    for spec in TARGETS:
