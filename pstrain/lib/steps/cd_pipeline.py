...
from pstrain.lib.dtree import build_tree, make_quests, prune_tree, tie_states
...
def run_init_cd_untied(
    ...
    # Write CD model
    _pstrainc.write_gau(str(output_dir / "means"), cd_means)
    _pstrainc.write_gau(str(output_dir / "variances"), cd_vars)
    _pstrainc.write_mixw(str(output_dir / "mixture_weights"), cd_mixw)
...
def run_init_cd_tied(
    ...
    Returns:
        Dict mapping model file types to paths
    """
    from pstrain.lib.dtree import init_mixw
