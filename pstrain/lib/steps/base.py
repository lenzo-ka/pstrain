...
(e.g. `pstrain step ci_hmm`). The full multi-step pipeline lives in
`pstrain.lib.pipeline`, which has its own task definitions.
...
class StepContext:
    ...
    # - pstrain.lib.features.extract_features() for feature extraction
    # - pstrain.lib.flat.init_flat_model() for flat model creation
    # - pstrain.lib.bw (TODO) for Baum-Welch training
